...
import ga.constants
import ga.summary
from ga.summary import ALL_TRANSACTIONS_VALUE
...
class ResponseTimeGroupHandler:
    ...
    def addData(self, columns):
        ...
        if self._responseTimeGroups.get(ALL_TRANSACTIONS_VALUE) is None:
...
def usage():
    logger.fatal('Usage: analyzer.py "<grinder data Files>" <mapping file> [number of agents]')
    sys.exit()
...
def main():
    ...
    global grinderDataFiles  # to be used to determine response time group
                             # multiplier
    grinderDataFiles=[]
    ...
    summaryData = ga.summary.SummaryDataRegistry(grinderOutFile)
    ...
    for transactionNumber in transactions.keys():
        ...
        meanMaxRTGrapher.saveChartToDisk(CONFIG.reportDir)
